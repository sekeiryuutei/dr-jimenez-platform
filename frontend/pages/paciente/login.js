...
export default function PatientLogin() {
  ...
  async function handleSubmit(e) {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${BACKEND_URL}/api/patient-auth/login`, {
        method: 'POST',
  ...
  return (
    <main className="panel-login-wrap">
      ...
      <form onSubmit={handleSubmit} className="panel-login-box">
        ...
        <p className="panel-login-hint">Usa el correo o la cédula con la que agendaste, y la contraseña que te enviamos.</p>
        ...
        <div style={{ textAlign: 'center', marginTop: 18 }}>
          <a href="/paciente/recuperar" style={{ fontSize: 12, color: '#7f9fa2' }}>¿Olvidaste tu contraseña?</a>
        </div>
      </form>
    </main>
  );
